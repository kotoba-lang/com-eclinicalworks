(ns eclinicalworks.validation
  ...
  Straight algorithmic ports (not a runtime dependency) of
  `hl7-fhir.validation` in kotoba-lang/com-hl7-fhir, where this logic was
  first ported into the deepen_actors.py-generated cljc template (ADR-2607083000
  / ADR-2607083100 / ADR-2607083200 EHR-compat increments). Ported by value
  -- each actor stays a standalone deploy unit -- rather than by adding a
  cross-repo dependency, so every clean-room healthcare actor in this cohort
  agrees on what counts as a structurally valid NPI / ICD-10-CM code /
  procedure code / GDPR Art. 9(2) lawful-basis point-letter / EHDS Art. 3
  access method.
...
;; (CELEX:32016R0679) and gdpr-info.eu (see hl7-fhir.validation, where this
;; list was first verified on 2026-07-08). Article 9 is one of the most
;; litigated/cited GDPR provisions and its (a)-(j) structure has been stable
;; since the Regulation's 2016 adoption, so this list is treated as
;; authoritative, not "representative only". It intentionally does not model
;; any EU Member State's national implementing law (e.g. German BDSG Sec. 22,
;; which adds conditions on top of (h)/(i)/(j) for health data specifically)
;; -- that is explicitly out of scope for this pass.
...
;;
;; Ported by value from `hl7-fhir.validation` in kotoba-lang/com-hl7-fhir
;; (ADR-2607083200), where the verbatim Article 3(1)-(3) text this validator
;; is based on was retrieved via a real-browser EUR-Lex session on
;; 2026-07-08 (EUR-Lex blocks automated fetches with an AWS WAF JS
;; challenge) and is archived at
...
;; priority-categories list), Article 4 (the "electronic health data access
;; services" definition) and Article 15 (the European electronic health
;; record exchange format). Article 14(1) was retrieved the same way on
;; 2026-07-09 and is archived at eu-ehds/ehds-article14-15-excerpt.md, so the
;; priority categories ARE now enumerated below (`ehds-priority-categories` /
;; `valid-ehds-priority-category?`, ported by value from `hl7-fhir.validation`
;; same as the rest of this section). Article 4 remains unretrieved. Article
