...
(def entity-specs
  [{:entity "Patient"   :plural "patients"   :id-prefix "eclinica_pat"
   ...
   ;; request (ported from kotoba-lang/com-hl7-fhir, ADR-2607083200). Article
   ;; 3(1)-(3) is the only portion of EHDS with a verified primary-source
   ;; reading to hand -- see orgs/kotoba-lang/emr-claims-primary-sources/
   ;; eu-ehds/ehds-article3-excerpt.md (verbatim excerpt, retrieved via a
   ;; real-browser EUR-Lex session on 2026-07-08, CELEX:32025R0327). Article
   ;; 14 (the priority-categories list) and Article 15 (the exchange-format
...
(defn handle-create [store entity data]
  (let [{:keys [fields required coerce id-prefix validate]
    ...
    (or (some-> (reject-unknown data (conj fields :id)) (vector 400))
