(ns eclinicalworks.main
  "Kotodama WASM entrypoint for the Eclinicalworks clean-room actor (L4) — Clojure port.
  ...
  facts (`eclinicalworks.<Entity>/<field>`); `*store*` is the in-memory materialization
  ...
  restriction/reason cross-field rule (see `eclinicalworks.validation`), all
  ported by value from kotoba-lang/com-hl7-fhir (ADR-2607083000 /
  ADR-2607083100 / ADR-2607083200) -- so a claim with a malformed billing
  NPI, a consent record citing a lawful-basis code that isn't one of the
  Regulation's ten, or an access request that flags a restriction without
  recording why, is rejected with 400 instead of silently persisted."
  (:require [clojure.string :as str]
            [eclinicalworks.validation :as validation]))

(def ns-prefix "eclinicalworks")
(def tier "L4")
...
(def entity-specs
  [{:entity "Patient"   :plural "patients"   :id-prefix "eclinica_pat"
   ...
   {:entity "Observation"      :plural "observations"      :id-prefix "eclinica_obs"
    :fields [:resourceType :status :effectiveDateTime :valueString :issued] :required [:resourceType]
    :coerce {}          :refs {}}
   {:entity "Encounter"     :plural "encounters"     :id-prefix "eclinica_enc"
    :fields [:resourceType :status :serviceType :period]        :required [:resourceType]
    :coerce {} :refs {}}
   {:entity "MedicationRequest" :plural "medicationrequests" :id-prefix "eclinica_med"
    :fields [:resourceType :status :intent :authoredOn]          :required [:resourceType]
    :coerce {}          :refs {}}
   {:entity "AllergyIntolerance" :plural "allergyintolerances" :id-prefix "eclinica_all"
    :fields [:resourceType :clinicalStatus :criticality :recordedDate]  :required [:resourceType]
    :coerce {}        :refs {}}
   {:entity "Condition" :plural "conditions" :id-prefix "eclinica_con"
   ...
   ;; billing-identifier formats via :validate (ported from
   ;; kotoba-lang/com-hl7-fhir, ADR-2607083000 EHR-compat L5->L6 increment):
   ;; billing provider NPI (Luhn check digit per 45 CFR 162.410), primary
   ;; diagnosis code (ICD-10-CM), procedure code (CPT Category I/II/III or
   ;; HCPCS Level II).
   {:entity "Claim" :plural "claims" :id-prefix "eclinica_cla"
    ...
    :refs {}}
   ;; EU GDPR Art. 9(2) special-category-data lawful-basis record (ported
   ;; from kotoba-lang/com-hl7-fhir, ADR-2607083100 EHR-compat EU increment).
   ;; Every clinical resource above carries "data concerning health" per
   ;; GDPR Art. 9(1) (Regulation (EU) 2016/679), which is prohibited unless
   ;; one of the ten Art. 9(2)(a)-(j) exceptions applies. This entity
   ;; records, per patient, which exception is being relied on.
   {:entity "Consent" :plural "consents" :id-prefix "eclinica_cst"
    ...
    :sample {:resourceType "Consent" :patientId "eclinica_pat_sample0000000000"
             :specialCategoryData true :lawfulBasisArt9 "h" :status "active"}
    :refs {}}
   ;; EU EHDS (Regulation (EU) 2025/327) Article 3 primary-use access
   ;; request (ported from kotoba-lang/com-hl7-fhir, ADR-2607083200;
   ;; priorityCategory enumerated 2026-07-09 once Article 14 was retrieved,
   ;; also ported by value from com-hl7-fhir). Article 3(1)-(3) and Article
   ;; 14(1) are the portions of EHDS with a verified primary-source reading
   ;; to hand -- see orgs/kotoba-lang/emr-claims-primary-sources/eu-ehds/
   ;; ehds-article3-excerpt.md and ehds-article14-15-excerpt.md (verbatim
   ;; excerpts, retrieved via a real-browser EUR-Lex session,
   ;; CELEX:32025R0327). `priorityCategory` is now the closed 6-value enum
   ;; Article 14(1)(a)-(f) lists verbatim. Article 15 (the European
   ;; electronic health record exchange format) is cross-referenced by
   ;; Article 3(2) but its technical schema is NOT modeled here: Article 15
   ;; itself delegates the concrete format to future European Commission
   ;; implementing acts, so `accessMethod` only cites Article 15 by name --
   ;; see README for the scope note.
   {:entity "PatientAccessRequest" :plural "patientaccessrequests" :id-prefix "eclinica_par"
    ...
    :sample {:resourceType "PatientAccessRequest" :patientId "eclinica_pat_sample0000000000"
...
(defn emit-facts
  "EAVT facts for one record: {\"eclinicalworks.<Entity>/<field>\" v ...}. The datomic
...
(defn handle-create [store entity data]
  (let [{:keys [fields required coerce id-prefix validate]
    ...
    (or (some-> (reject-non-map data) (vector 400))
        (some-> (reject-unknown data (conj fields :id)) (vector 400))
...
(defn healthz [] [{:status "ok" :actor "eclinicalworks-compat" :tier tier :entities entities} 200])
...
(defn start! [] :eclinicalworks-compat/ready)
