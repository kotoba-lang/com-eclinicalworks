(ns eclinicalworks.main-test
  "Contract + behavioral test for the eclinicalworks-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "eclinicalworks-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

;; --- Claim: US billing-identifier format validation (ported from
;; kotoba-lang/com-hl7-fhir, ADR-2607083000) -------------------------------
...
(deftest claim-domain-validation
  (testing "a fully valid claim is accepted"
    (let [s (m/fresh-store) [rec status] (m/handle-create s "Claim" claim-sample)]
      (is (= 201 status))
      (is (str/starts-with? (:id rec) "eclinica_cla_"))))
...
;; --- Consent: EU GDPR Art. 9(2) lawful-basis validation (ported from
;; kotoba-lang/com-hl7-fhir, ADR-2607083100) --------------------------------
...
(deftest consent-domain-validation
  (testing "a fully valid consent record is accepted"
    (let [s (m/fresh-store) [rec status] (m/handle-create s "Consent" consent-sample)]
      (is (= 201 status))
      (is (str/starts-with? (:id rec) "eclinica_cst_"))
...
;; --- PatientAccessRequest: EU EHDS Article 3 validation (ported from
;; kotoba-lang/com-hl7-fhir, ADR-2607083200) --------------------------------
...
(deftest patient-access-request-domain-validation
  (testing "a fully valid view request is accepted"
    (let [s (m/fresh-store) [rec status] (m/handle-create s "PatientAccessRequest" access-request-sample)]
      (is (= 201 status))
      (is (str/starts-with? (:id rec) "eclinica_par_"))
...
#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'eclinicalworks.main-test)]
